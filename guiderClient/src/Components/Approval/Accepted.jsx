...
const Accepted = () => {
    ...
    const filteredOrderData = orderData.filter(order => order.status === "Booked" || order.status === "Started").slice().reverse();
        ...
        <>
            {
                ...
                <div
                    ...
                    className={`flex flex-col w-[95vw]  text-black md:max-w-[45rem] p-4 bg-white rounded-lg shadow-[4px_5px_10px_-8px_#808080] border-l-4 ${filteredOrderData[0].status === 'Completed' ? 'border-green-500' : filteredOrderData[0].status === 'Started' || filteredOrderData[0].status === 'Booked' ? 'border-orange-500' : filteredOrderData[0].status === 'Cancelled' ? 'border-red-500' : 'border-blue-500'}`}
