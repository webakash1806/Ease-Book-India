...
const register = async (req, res, next) => {
    ...
    try {
        ...
        const uniquePhoneNumber = await Cars.findOne({ phoneNumber })
        ...
        cars.proofFiles = await uploadedFiles

        // Generating JWT token and setting it as a cookie
        const token = await cars.generateJWTToken()
        res.cookie('token', token, cookieOption)

        // Saving Cars details and sending response to the client
        if (password === confirmPassword) {
            await cars.save()
            cars.password = undefined
            cars.confirmPassword = undefined
            res.status(201).json({
                success: true,
                message: 'Cars registered Successfully',
                cars
            })
        }
        else {
