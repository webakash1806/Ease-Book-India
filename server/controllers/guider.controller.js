...
const register = async (req, res, next) => {
    try {
        ...
        // Extracting Guider input from request body
        const { fullName, email, password, confirmPassword, phoneNumber, experience, age } = req.body

        // Validating required fields
        if (!fullName || !phoneNumber || !email || !password || !confirmPassword || !experience || !age) {
            return next(new AppError('All Fields are required', 400))
        }

        // Checking if the email is already registered
        const uniqueEmail = await Guider.findOne({ email })
        if (uniqueEmail) {
            return next(new AppError('Email is already registered', 400))
        }

        const uniquePhoneNumber = await Guider.findOne({ phoneNumber })

        if (uniquePhoneNumber) {
            return next(new AppError('Phone number is already registered', 400))
        }

        // Creating a new Guider in the database
        const priest = await Guider.create({
            fullName,
            email,
            experience,
            age,
        ...
        priest.proofFiles = await uploadedFiles

        // Generating JWT token and setting it as a cookie
        const token = await priest.generateJWTToken()
        res.cookie('token', token, cookieOption)

        // Saving Guider details and sending response to the client
        if (password === confirmPassword) {
            await priest.save()
            priest.password = undefined
            priest.confirmPassword = undefined
            res.status(201).json({
                success: true,
                message: 'Guider registered Successfully',
                priest
            })
        }
        else {
...
const login = async (req, res, next) => {
    try {
        ...
        const priest = await Guider.findOne({
            email
        }).select('+password')
        ...
        const passwordCheck = await priest.comparePassword(password)
        if (!passwordCheck) {
...
const profile = async (req, res, next) => {
    try {
        ...
        const priest = await Guider.findById(userId)
...
const forgotPassword = async (req, res, next) => {
    ...
    const priest = await Guider.findOne({ email })
    ...
    const resetToken = await priest.generatePasswordResetToken()
    await priest.save()
    ...
    try {
        ...
        priest.forgetPasswordExpiry = undefined
        priest.forgetPasswordToken = undefined
...
const resetPassword = async (req, res, next) => {
    try {
        ...
        priest.password = await bcrypt.hash(password, 10)
        priest.forgetPasswordToken = undefined
        priest.forgetPasswordExpiry = undefined
...
const changePassword = async (req, res, next) => {
    try {
        ...
        const priest = await Guider.findById(id).select('+password')
        ...
        const passwordValid = await priest.comparePassword(oldPassword)
        ...
        priest.password = await bcrypt.hash(newPassword, 10)
        await priest.save()
...
const updateProfile = async (req, res, next) => {
    try {
        ...
        // Finding the user by ID
        const priest = await Guider.findById(id)
        ...
        if (fullName) {
            priest.fullName = await fullName
        }

        if (phoneNumber) {
            priest.phoneNumber = await phoneNumber
        }
...
const addService = async (req, res, next) => {
    try {
        const { placesList, serviceArea, availability, fare } = req.body
        const id = req.user.id
        console.log(id)
        const user = await Guider.findById(id)
        if (!placesList || !serviceArea || !availability || !fare) {
            return next(new AppError('All fields are required', 400))
        }

        user.servicesData.fare = await fare
        user.servicesData.placesList = await placesList
