...
const register = async (req, res, next) => {
    try {
        ...
        // Extracting Priest input from request body
        ...
        const uniqueEmail = await Priest.findOne({ email })
        if (uniqueEmail) {
            return next(new AppError('Email is already registered', 400))
        }

        const uniquePhoneNumber = await Priest.findOne({ phoneNumber })

        if (uniquePhoneNumber) {
            return next(new AppError('Phone number is already registered', 400))
        }

        // Creating a new Priest in the database
        ...
        // Saving Priest details and sending response to the client
        if (password === confirmPassword) {
            ...
            res.status(201).json({
                success: true,
                message: 'Priest registered Successfully',
                priest
            })
        }
        else {
...
const profile = async (req, res, next) => {
    try {
        ...
        const priest = await Priest.findById(userId)
...
const changePassword = async (req, res, next) => {
    try {
        ...
        const priest = await Priest.findById(id).select('+password')
...
const getPriestWithService = async (req, res, next) => {
    try {

        const allPriest = await Priest.find()

        const filteredPriest = allPriest.filter(driver => driver.servicesData.poojaList != [] && driver.status !== "REJECTED")
