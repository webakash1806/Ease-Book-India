...
const Register = () => {
    ...
    const handleForm = async (e) => {
        ...
        const { fullName, email, password, confirmPassword, phoneNumber, boatType, boatNumber, age, experience } = input


        if (!fullName || !email || !password || !confirmPassword || !phoneNumber || !boatType || !boatNumber || !age || !experience) {
        ...
        formData.append('boatNumber', boatNumber)
            ...
            <form onSubmit={handleForm} className='sm:p-10 p-6  bg-[#2F3349] text-white rounded-md shadow-[0px_0px_20px_#3D4056] my-12 flex md:items-end md:flex-row flex-col w-fit md:gap-10'>
                <div className='flex flex-col gap-2'>
                    ...
                    <div className={mainDiv}>
                        ...
                        <input className={inputStyle} type="text" name='boatType' value={input.boatType} onChange={handleUserInput} id='boatType' />
                    </div>
                    <div className={mainDiv}>
                        <label className={labelStyle} htmlFor="boatNumber">Boat Number</label>
                        <input className={inputStyle} type="text" name='boatNumber' value={input.boatNumber} onChange={handleUserInput} id='boatNumber' />
                    </div>
                ...
                <div className='flex flex-col gap-2 mt-2 md:mt-20'>
                    ...
                    <div className='flex  min-w-[18rem] md:max-w-[20.5rem] max-w-[25.5rem] w-[87vw] sm:w-[24rem] justify-between'>
                        ...
                        <div className={`${mainDiv} w-[48%]`}>
                            <label className={labelStyle} htmlFor="boatImage">Image of Boat</label>
                            <input className='border border-[#685ED4] w-full rounded-[3px] h-full  px-2 p-[5.5px]  outline-none  text-[0.95rem] tracking-wide resize-none bg-[#3D4056] text-white' type="file"
                    ...
                    <div className={mainDiv}>
                        <label className={labelStyle} htmlFor="DL">Driving license</label>
                        <input className={inputStyle} type="file" name="DL"
                            id="DL"
                            onChange={(e) => handleFileChange(e, 4)} />
                    </div>
                    <div className={mainDiv}>
                        <label className={labelStyle} htmlFor="boatCertificate">BOAT Certificate</label>
                        <input className={inputStyle} type="file" name="boatCertificate"
                            id="boatCertificate"
